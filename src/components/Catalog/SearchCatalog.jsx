...
export const SearchCatalog= ({categories, goodsAll})=> {
    ...
    const onFetchSearch = async (categoryId)=>{
        const resp = await fetch(`${BASE_URL}/api/items?categoryId=${categoryId}&q=${search.title} ` );
        const data = await resp.json();
        setPreviewList(data)
    ...
    const onSearchChange = (e)=> {
        ...
        setTimeout(()=> onFetchSearchAll(), 1500 )
        } else {
            setTimeout(()=> onFetchSearch(selected.id), 1500 )
        }    
    }
